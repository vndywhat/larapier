...
{
    // Announce Intervals
    private const MIN = 240;
    private const MAX = 360;
    ...
    protected function checkClient(Request $request): void
    {
        if(! $request->header('User-Agent')) {
            throw new AnnounceException(120);
        }

        $userAgent = $request->header('User-Agent');

        // Should Block those too long User-Agent. ( For Database reason
        if (\strlen($userAgent) > 64) {
            throw new AnnounceException(123);
        }
        ...
        if (\preg_match('/(Mozilla|Browser|Chrome|Safari|AppleWebKit|Opera|Links|Lynx|Bot|Unknown)/i', $userAgent)) {
            throw new AnnounceException(121);
        }
    }
    ...
    protected function checkPasskey($passkey): void
    {
        ...
        if ($passkey === null) {
            throw new AnnounceException(130, [':attribute' => 'passkey']);
        }

        // If Passkey Lenght Is Wrong
        if (\strlen($passkey) !== 32) {
            throw new AnnounceException(132, [':attribute' => 'passkey', ':rule' => 32]);
        }

        // If Passkey Format Is Wrong
        if (\strspn(\strtolower($passkey), 'abcdef0123456789') !== 32) {  // MD5 char limit
            throw new AnnounceException(131, [':attribute' => 'passkey', ':reason' => 'The format of passkey isnt correct']);
        }
    }
    ...
    private function checkAnnounceFields(Request $request): array
    {
        ...
        foreach (['info_hash', 'peer_id', 'port', 'uploaded', 'downloaded', 'left'] as $item) {
            ...
            if (! \is_null($itemData)) {
                ...
                throw new AnnounceException(130, [':attribute' => $item]);
            }
        }

        foreach (['info_hash', 'peer_id'] as $item) {
            if (\strlen($queries[$item]) !== 20) {
                throw new AnnounceException(133, [':attribute' => $item, ':rule' => 20]);
            }
        }
        ...
        foreach (['uploaded', 'downloaded', 'left'] as $item) {
            ...
            if (! \is_numeric($itemData) || $itemData < 0) {
                throw new AnnounceException(134, [':attribute' => $item]);
            }
        }
        ...
        if (! \in_array(\strtolower($queries['event']), ['started', 'completed', 'stopped', 'paused', ''])) {
            throw new AnnounceException(136, [':event' => \strtolower($queries['event'])]);
        }
        ...
        if ($queries['port'] === 0 && \strtolower($queries['event']) !== 'stopped') {
            throw new AnnounceException(137, [':event' => \strtolower($queries['event'])]);
        }

        if (! \is_numeric($queries['port']) || $queries['port'] < 0 || $queries['port'] > 0xffff || \in_array($queries['port'], self::BLACK_PORTS,
                true)) {
            throw new AnnounceException(135, [':port' => $queries['port']]);
        }
    ...
    private function checkMinInterval($queries, $user): void
    {
        ...
        if ($prevAnnounce < $carbon->copy()->subSeconds(self::MIN)->toDateTimeString() && \strtolower($queries['event']) !== 'completed') {
            throw new AnnounceException(162, [':min' => self::MIN]);
        }
    }
    ...
    private function checkMaxConnections($torrent, $user): void
    {
        ...
        if ($connections > \config('announce.rate_limit')) {
            throw new AnnounceException(138, [':limit' => \config('announce.rate_limit')]);
        }
    }
    ...
    /**
     * @param AnnounceException $announceException
     *
     * @return array
     */
    protected function generateFailedAnnounceResponse(AnnounceException $announceException): array
    {
        return [
            'failure reason' => $announceException->getMessage(),
            'min interval'   => self::MIN,
            /**
    ...
    public function index(Request $request, $passkey)
    {
        try {
            ...
            Log::info('info_hash ' . $queries['info_hash']);
            ...
            /**
             * Dispatch The Specific Announce Event Job.
             */
